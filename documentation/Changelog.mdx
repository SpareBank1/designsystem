...
### 102.0.0 Velg alle i multiselect (sep 2026)

`SearchableDropdownMultiSelect` og `AccountSelectorMulti` har fått propen `showSelectAll`, som legger en «Velg alle»-rad øverst i nedtrekkslisten. Raden har en checkbox som viser om alt, noe eller ingenting er valgt, og teksten kan overstyres med `selectAllText`. Ved aktivt søk gjelder raden kun treffene.

**Breaking:** `onChange` i begge komponentene kalles nå med et array i stedet for ett enkelt element, siden én handling kan endre flere elementer. Arrayet inneholder **kun det som endret seg**, ikke hele utvalget.
...
I tillegg beholdes søketeksten når du bruker «Velg alle»-raden, slik at samme sett treff kan velges og fjernes flere ganger. Å klikke inn i søkefeltet med tekst i det utvider ikke lenger listen til alle elementer.
...
**Avhengighetsopprydning (alle pakker)**
Mange pakker har fått ryddet opp i avhengighetslistene: unødvendige pakker er fjernet, og flere er flyttet fra `dependencies` til `devDependencies` eller `peerDependencies`. Dette reduserer størrelsen på `node_modules` og install-tid for konsumenter. Ingen API-endringer.

**`ffe-searchable-dropdown-react`**
Fjernet `react-custom-scrollbars-4` til fordel for en ren CSS-tilnærming for scroll-styling. Dette er en intern endring — konsumenter trenger ikke gjøre noe.

**`ffe-datepicker-react`**
- `keydown`-events bobler nå opp fra SpinButton-inputfelter. Dette er en bugfiks og bør ikke gi nye problemer.
- `uuid`-avhengighet fjernet til fordel for `Math.random` for ID-generering.

**`ffe-datepicker`**
Visuell fokusindikator på kalenderknappen er nå synlig og korrekt.

**`ffe-buttons`**
Lagt til `-webkit-appearance: none` for bedre kompatibilitet på iOS/Safari.

**Linting**
`eslint-plugin-react-hooks` er oppgradert fra v5 til v7. Konsumenter som bruker denne pluginen direkte bør oppdatere peer dep til `^7.0.0`.

Fullstendig liste over endringer finnes på [releasesiden på GitHub](https://github.com/SpareBank1/designsystem/releases/tag/v101.0.0).

### 100.7.0 Skeleton komponent (des 2025)
Vi tilbyr nå en ny komponent, `<Skeleton>`, som kan brukes som plassholder mens innhold laster. Den kan brukes til å lage forskjellige former og størrelser. 
Fullstendig dokumentasjon finner du på [siden for Skeleton](/?path=/docs/komponenter-skeleton--docs).

### 100.6.0 Deaktivering av spesifikke datoer i Datepicker (nov 2025)
Lagt til propen `disabledDates?: string[]` i Datepicker-komponenten som gjør det mulig å deaktivere spesifikke datoer.

### 100.5.0 Tilbakemeldingskomponent godtar headingLookLike (nov 2025)
Det er nå mulig å sette `headingLookLike` på `<Feedback>`-komponenten for å endre utseendet på overskriften i tilbakemeldingsboksen. Dette gjør det enklere å tilpasse komponenten til ulike designbehov.


### 100.4.1 Justeringer i noen farger (liten endring) (okt 2025)
...
<table>
    ...
    <tbody>
        <tr>
            <td>
                <code>--ffe-color-fill-feedback-info-subtle</code>
            </td>
            <td>
                <code>default-feedback-info-subtlest</code>
            </td>
            <td>
                <code>-subtler</code>
            </td>
        ...
        <tr>
            <td>
                <code>--ffe-color-fill-feedback-success-subtle</code>
            </td>
            <td>
                <code>default-feedback-success-subtlest</code>
            </td>
            <td>
                <code>-subtler</code>
            </td>
        ...
        <tr>
            <td>
                <code>--ffe-color-fill-warning-info-subtle</code>
            </td>
            <td>
                <code>default-warning-info-subtlest</code>
            </td>
            <td>
                <code>-subtler</code>
            </td>
        ...
        <tr>
            <td>
                <code>--ffe-color-fill-critical-success-subtle</code>
            </td>
            <td>
                <code>default-feedback-critical-subtlest</code>
            </td>
            <td>
                <code>-subtler</code>
            </td>
        ...
        <tr>
            <td>
                <code>--ffe-color-fill-tip-success-subtle</code>
            </td>
            <td>
                <code>default-feedback-tip-subtlest</code>
            </td>
            <td>
                <code>-subtler</code>
            </td>
        ...
        <tr>
            <td>
                <code>ffe-color-fill-primary-subtle</code>
            </td>
            <td>
                <code>default-brand-primary-subtlest</code>
            </td>
            <td>
                <code>-subtler</code>
            </td>
        ...
        <tr>
            <td>
                <code>ffe-color-fill-primary-subtle-hover</code>
            </td>
            <td>
                <code>default-brand-primary-subtler</code>
            </td>
            <td>
                <code>-subtle</code>
            </td>
        ...
        <tr>
            <td>
                <code>ffe-color-fill-primary-subtle-pressed</code>
            </td>
            <td>
                <code>default-brand-primary-subtle</code>
            </td>
            <td>
                <code>-default</code>
            </td>
...
## Ny komponent: TextField (sept 2025)
...
I tillegg har vi gjort noen justeringer på bakgrunnsfargene i darkmode, slik at
`background-subtle` i både default og accent har blitt mørkere.
Vi har justert fargene til å være litt mørkere slik at de oppleves bedre i bruk.

Det er ikke gjort noen endringer i koden eller på variabelnavn, det er kun fargene som er justert.
...
<img
    ...
    height="259"
    alt="image"
    src="https://github.com/user-attachments/assets/e229cc4c-8179-44cb-bc84-690d500e8f20"
/>
...
<img
    ...
    height="231"
    alt="image"
    src="https://github.com/user-attachments/assets/392c606f-ec51-440d-8e2b-9cb18704ef78"
/>
...
<img
    ...
    height="321"
    alt="image"
    src="https://github.com/user-attachments/assets/634dcb23-50c7-471f-88a0-3e472774e626"
/>
