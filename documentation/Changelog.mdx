...
`SearchableDropdownMultiSelect` og `AccountSelectorMulti` har fått propen `showSelectAll`, som legger en «Velg alle»-rad øverst i nedtrekkslisten. Teksten bytter til «Fjern alle» når alt som vises er valgt, og begge tekstene kan overstyres med `selectAllTexts`. Ved aktivt søk gjelder raden kun treffene.
