...
const packageStyles = {
    ...
    '@sb1/ffe-collapse-react': { less: '@sb1/ffe-collapse-react/less/collapse' },
    '@sb1/ffe-core': { less: '@sb1/ffe-core/less/ffe' },
    '@sb1/ffe-icons': { less: '@sb1/ffe-icons/less/ffe-icons' },
    '@sb1/ffe-searchable-dropdown-react': { less: '@sb1/ffe-searchable-dropdown-react/less/searchable-dropdown' },
    ...
    '@sb1/ffe-chart-donut-react': { less: '@sb1/ffe-chart-donut-react/less/ffe-chart-donut' },
...
const reactPackages = fs.readdirSync(packagesDir)
    .filter(dir => dir.endsWith('-react') && dir !== 'ffe-grid-react') // Exclude grid
    .map(dir => path.join(packagesDir, dir));

function getCssImports(packageName, dependencies) {
    const allPackages = [packageName, ...dependencies].filter(pkg => pkg !== '@sb1/ffe-grid' && pkg !== '@sb1/ffe-grid-react');
    const cssImports = allPackages
        .map(dep => packageStyles[dep])
        .filter(Boolean)
        .sort((a, b) => 
            a.less === '@sb1/ffe-core/less/ffe' ? -1 :
            b.less === '@sb1/ffe-core/less/ffe' ? 1 :
...
function extractTypeInfo(tsxContent, componentName) {
    ...
    if (propsMatch) {
        ...
        for (const line of propLines) {
            const propMatch = line.match(/(?:\/\*\*\s*(.+?)\s*\*\/\s*)?(\w+)(\??)\s*:\s*([^;]+)/);
            if (propMatch) {
    ...
    const jsdocPattern = /\/\*\*\s*([\s\S]*?)\s*\*\/\s*(?:export\s+)?(?:const|function)\s+\w+/;
...
function findComponentFiles(packageDir) {
    ...
    return files
        .filter(f => f.endsWith('.tsx') && !f.includes('.spec.') && !f.includes('.stories.'))
...
function extractUsageInfo(mdxContent, readmeContent) {
    ...
    // Try to extract description from MDX
    if (mdxContent) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Skip empty lines, imports, and JSX after title
            if (foundTitle && !foundDescription) {
                if (!line || line.startsWith('import ') || line.startsWith('<') || 
                    line.startsWith('```') || line.startsWith('##')) {
                    continue;
                }
                // Found a real text line - this is the description
                if (line.length > 10) {
                    description = line;
        ...
        description = description.replace(/<[^>]+>/g, '').replace(/packageName=/g, '').replace(/["']/g, '').replace(/[{}\[\]]/g, '').trim();
        
        // Look for usage guidelines
        const usageSection = mdxContent.match(/##\s+Bruk av[^#]*/i);
        if (usageSection) {
            whenToUse = usageSection[0];
        }
        
        // Look for "Gjør sånn" and "Ikke sånn" sections
        ...
        // Look for "Når skal komponenten brukes?" section
        const narBrukes = mdxContent.match(/##\s+Når skal komponenten brukes\?[^#]*/i);
        if (narBrukes) {
            whenToUse += '\n\n' + narBrukes[0];
        }
    }
    
    // Fallback to README if no good description found
    if (!description || description.length < 10) {
        if (readmeContent) {
            const readmeMatch = readmeContent.match(/^##\s+Usage\n\n([^\n]+)/m);
            if (readmeMatch) {
                description = readmeMatch[1].replace(/Full documentation.*/, '').trim();
            }
        }
    }
...
function findMainMdxFile(packageDir) {
    ...
    const mainFiles = files.filter(f => 
        f.endsWith('.mdx') && 
        (f.includes('s.mdx') || f === path.basename(packageDir).replace('-react', '').replace('ffe-', '') + '.mdx')
    );
...
function getExportedComponents(packageDir) {
    ...
    for (const match of exportMatches) {
        const items = match[1].split(',').map(s => s.trim().split(' as ')[0]).filter(Boolean);
        exports.push(...items);
    }
    
    // Also match direct exports
    const directExports = content.matchAll(/export\s+.*\s+(function|const|class)\s+(\w+)/g);
...
function generateCodeExample(packageName, mainComponents, dependencies) {
    ...
    const simpleName = componentName.replace('Props', '').replace(/^type\s+/, '');
...
function generateAiContext(packageDir) {
    ...
    if (fs.existsSync(depsPath)) {
        dependencies = JSON.parse(fs.readFileSync(depsPath, 'utf-8'))
            .filter(dep => dep !== '@sb1/ffe-grid' && dep !== '@sb1/ffe-grid-react');
    }
    ...
    const { description, whenToUse, whenNotToUse } = extractUsageInfo(mdxContent, readmeContent);
    ...
    const installCmd = dependencies.length > 0 
        ? `npm install ${packageName} ${dependencies.join(' ')}`
        : `npm install ${packageName}`;
    
    // Find main component names (not Props types)
    const mainComponents = components.filter(c => !c.includes('Props') && !c.includes('type'));
    ...
    if (mainComponents.length > 0) {
        content += `## Basic Usage\n\n`;
        content += generateCodeExample(packageName, mainComponents, dependencies);
        content += '\n\n';
    }
    
    // Add API Reference for main components
    if (Object.keys(typeInfos).length > 0) {
        content += `## API Reference\n\n`;
        
        for (const [componentName, typeInfo] of Object.entries(typeInfos)) {
            if (typeInfo.props.length > 0) {
                content += `### ${componentName} Props\n\n`;
                
                if (typeInfo.jsdoc) {
                    content += `${typeInfo.jsdoc}\n\n`;
                }
                
                content += `| Prop | Type | Required | Description |\n`;
                content += `|------|------|----------|-------------|\n`;
                
                for (const prop of typeInfo.props) {
...
reactPackages.forEach(packageDir => {
    const packageName = '@sb1/ffe' + path.basename(packageDir).replace('ffe', '');
    const shortName = path.basename(packageDir).replace('ffe-', '').replace('-react', '');
    ...
    const packageOutputPath = path.join(packageDir, 'ai-context.md');
    fs.writeFileSync(packageOutputPath, content, 'utf-8');
    console.log(`  ✓ Created ${packageOutputPath}\n`);
    
    // Add to resources list
    resources.push({
        uri: `ffe://component/${shortName}`,
        name: packageName,
        description: content.split('\n').find(line => line.startsWith('## Description'))?.split('\n')[2] || '',
    ...
    if (mdxPath) {
        ...
        const info = extractUsageInfo(mdxContent, '');
    ...
    packageSummaries.push({
        name: packageName,
        description,
        components: getExportedComponents(packageDir).filter(c => !c.includes('Props') && !c.includes('type')),
    });
});
...
const categories = {
    'Form & Input': ['form', 'datepicker', 'dropdown', 'searchable-dropdown', 'file-upload', 'account-selector'],
    'Buttons & Actions': ['buttons'],
    'Layout & Structure': ['cards', 'accordion', 'collapse', 'tabs'],
    'Feedback & Messages': ['messages', 'message-box', 'context-message', 'system-message', 'feedback', 'modals'],
    'Navigation & Controls': ['pagination', 'chips', 'tags'],
    'Data Display': ['tables', 'lists', 'progressbar', 'spinner', 'chart-donut'],
...
Object.entries(categories).forEach(([category, keywords]) => {
    ...
    matchingPackages.forEach(pkg => {
        ...
        const shortName = pkg.name.replace('@sb1/', '').replace('ffe-', '').replace('-react', '');
        rootContent += `[View detailed documentation](./packages/ffe-${shortName}-react/ai-context.md)\n\n`;
    });
});
...
rootContent += `2. Navigate to its package-specific ai-context.md file\n`;
...
const rootOutputPath = path.join(__dirname, '../ai-context.md');
fs.writeFileSync(rootOutputPath, rootContent, 'utf-8');
console.log(`✓ Created ${rootOutputPath}\n`);
...
const resourcesPath = path.join(__dirname, '../ai-context-resources.json');
fs.writeFileSync(resourcesPath, JSON.stringify(resourcesManifest, null, 2), 'utf-8');
console.log(`✓ Created ${resourcesPath}\n`);

console.log('Done! Generated enhanced AI context documentation.');
console.log(`Total components documented: ${resources.length - 1}`);
console.log(`Resources manifest: ${resourcesPath}`);
