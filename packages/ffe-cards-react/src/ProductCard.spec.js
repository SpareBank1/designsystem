...
describe('<ProductCard />', () => {
    ...
    it('renders status if provided', () => {
        ...
        expect(wrapper.find('.ffe-product-card__status').text()).toBe('Fornyelse');
    });
    it('renders icon if provided', () => {
        const wrapper = getWrapper({ icon: <svg className="some-icon" /> });
        expect(wrapper.find('.ffe-product-card__icon').length).toBe(1);
        expect(wrapper.find('.ffe-product-card__icon').hasClass('some-icon')).toBe(true);
    });
    it('renders heading if provided', () => {
        ...
        expect(wrapper.find('.ffe-product-card__heading').text()).toBe('Heading');
    });
    it('renders details if provided', () => {
        ...
        expect(wrapper.find('.ffe-product-card__details').text()).toBe('Details');
    });
    it('passes props as expected', () => {
        const wrapper = getWrapper({ role: 'button', className: 'kerfuffle' });
        expect(wrapper.find('.ffe-product-card').hasClass('kerfuffle')).toBe(true);
