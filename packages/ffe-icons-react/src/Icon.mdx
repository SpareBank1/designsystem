...
riktig håndtert av byggeverktøyet. Her er det to løsningsforslag. Et er å legge til `build.assetsInlineLimit: 0` i `vite.config.ts` 
for å sikre at SVG-ikonene ikke blir inlinet som data-URL-er. Merk at dette vil stoppe alt av assetinlining som Vite kan gjøre under bygg.
Et annet er å legge til `?no-inline` på slutten av importsti, eksempelvis vil import se slik ut:

```
import stacksIcon from "@sb1/ffe-icons/icons/open/500/sm/stacks.svg?no-inline"
```

Det må i så fall gjøres alle steder hvor du ønsker å bruke `<Icon/>`.
