...
## Mobiltilpasning - forskjellige breakpoints
Tabellen tilpasser seg mindre skjermstørrelser ved å sette header og tekst sammen. 
Du kan justere når dette skjer ved å sende inn `breakPoint?: 'none' | 'sm' | 'md'` (merk stor P). 
`sm` er default. 

<Canvas of={TableStories.DifferentBreakPoint} />
<Controls of={TableStories.DifferentBreakPoint} /> 