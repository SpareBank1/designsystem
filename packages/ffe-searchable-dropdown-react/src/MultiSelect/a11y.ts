...
const getItemSelectedMessage = ({
    ...
    return getItemSelectedA11yStatus(locale, 'test'); //Fix with a11y message
};
...
>({
    ...
    const selectedItemValues: string[] = selectedItems?.map(
        item => item[dropdownAttributes[0]],
    );
    ...
    useEffect(() => {
        ...
        const equal = stringArraysEqual(
            selectedItemValues,
            prevSelectedItemValue.current,
        );
        const selectedItemHasChanged = !equal;
        if (selectedItemHasChanged) {
            prevSelectedItemValue.current = selectedItemValues;
            updateA11yStatus(() => {
                return getItemSelectedMessage({ selectedItemValues, locale });
            });
        } else {
            updateA11yStatus(() => {
